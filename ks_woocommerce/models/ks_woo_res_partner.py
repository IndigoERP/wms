...
class KsWooResPartner(models.Model):
    ...
    ks_res_partner = fields.Many2one("res.partner", string="Odoo Partner", readonly=True, ondelete='cascade', help="Displays Odoo related record name")
    ks_company_id = fields.Many2one("res.company", string="Company", compute="_compute_company", store=True, help="Displays Company Name")
    ...
    def create_woo_record(self, instance, res_partner, export_to_woo=False, queue_record=False):
        ...
        try:
            ...
            if not customer_exist:
                ...
                if export_to_woo:
                    try:
                        layer_customer.ks_manage_woo_customer_export()
    ...
    def ks_manage_woo_customer_import(self, instance, partner_json, queue_record=False):
        """
        :param instance: ks.woo.connector.instance()
    ...
    def ks_convert_odoo_compatible_data(self, data, type=False):
        """
    ...
    def ks_prepare_export_json_data(self, customer=False):
        ...
        if not self.ks_res_partner.child_ids:
            ...
            partner_id = self.ks_res_partner.ks_partner_ids.search([('ks_wc_instance', '=', self.ks_wc_instance.id), ('id', '=', self.id)]).ks_res_partner
