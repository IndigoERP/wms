...
class KsWooCommerceConnectorInstance(models.Model):
    ...
    ks_language = fields.Many2one('res.lang', string='Language', help=_("Select language for your Woo Instance"))
    ...
    ks_invoice_tax_account = fields.Many2one('account.account', string="Invoice TAX Account", help="Show the tax account which will be used for invoice tax default account")
    ks_credit_tax_account = fields.Many2one('account.account', string="Credit Note TAX Account", help="Show the tax account which will be used for Credit Note/Refund tax default account")
    ...
    def ks_manage_auto_job(self):
        ...
        #     auto_update_stock_value = {
        #         'name': '[' + str(
        #             self.id) + '] - ' + self.ks_instance_name + ': ' + 'WooCommerce Auto Product and Stock Update from Odoo to Woo (Do Not Delete)',
        #         'interval_number': 1,
        #         'interval_type': 'days',
        ...
        #         'ks_wc_instance': self.id,
        #     }
        #     self.ks_aus_cron_id = self.env['ir.cron'].create(auto_update_stock_value)
        #     self.ks_aus_cron_id.code = 'model.ks_auto_update_wc_product_stock(' + str(self.ks_aus_cron_id.id) + ')'
    ...
    def ks_open_instance_logs(self):
        ...
        return {
            ...
            'view': 'form',
            'res_id': self.id,
            'res_model': 'ks.woo.logger',
            'domain': [('ks_woo_instance.id', '=', self.id)],
            'view_mode': 'tree,form',
        }
    ...
    def ks_open_woo_products(self):
        ...
        :return: action
        """
        action = self.env.ref('ks_woocommerce.action_ks_woo_product_template_').read()[0]
        action['domain'] = [('ks_wc_instance', '=', self.id)]
        return action
    ...
    def ks_open_woo_coupons(self):
        ...
        return {
            ...
            'domain': [('ks_wc_instance', '=', self.id)],
            'res_id': self.id,
            'res_model': 'ks.woo.coupons',
            'view_mode': 'tree,form',
        }
    ...
    def ks_open_woo_customers(self):
        ...
        :return: action
        """
        action = self.env.ref('ks_woocommerce.action_ks_woo_partner').read()[0]
        action['domain'] = [('ks_wc_instance', '=', self.id)]
        return action
    ...
    def open_sales_analysis(self):
        ...
        :return: action
        """
        action = self.env.ref('ks_woocommerce.action_sales_report_all').read()[0]
        action['domain'] = [('ks_wc_instance', '=', self.id)]
        return action
    ...
    def open_payment_method(self):
        ...
        :return: action
        """
        action = self.env.ref('ks_woocommerce.action_woo_payment_gateway').read()[0]
        action['domain'] = [('ks_wc_instance', '=', self.id)]
        return action
    ...
    def open_waiting_available(self):
        ...
        :return: action
        """
        action = self.env.ref('ks_woocommerce.action_woocommerce_deliveries').read()[0]
    ...
    def open_invoice(self):
        ...
        :return: action
        """
        action = self.env.ref('ks_woocommerce.action_woocommerce_invoices').read()[0]
