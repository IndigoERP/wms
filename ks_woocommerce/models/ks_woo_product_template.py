...
class KsWooProductTemplate(models.Model):
    ...
    # ks_wc_category_ids = fields.Many2many('ks.woo.product.category', string='Categories',
    ...
    def ks_manage_woo_product_template_import(self, instance, product_json_data, queue_record=False):
        """
                ...
                if product_exist:
                    try:
                        if product_exist.ks_woo_product_type == product_json_data.get("type"):
                            ...
                            if instance and instance.ks_want_maps:
                                if 'barcode_check_field' in main_product_data.keys():
                                    ...
                                    if product_barcode_exist:
                                        same_prod_tmpl_id = self.env['ks.woo.product.template'].search([('ks_woo_product_id','=', product_json_data.get('id')), ('ks_wc_instance','=', instance.id)]).ks_product_product
                            ...
                            # self.ks_manage_variant_images(product_exist.ks_product_template, instance,
                            #                               product_json_data)
                        ...
                        if product_exist.ks_woo_product_type == product_json_data.get("type"):
                            if instance and instance.ks_want_maps:
                                ...
                                for each_var in all_woo_variations:
                                    product_barcode_exist = self.check_similar_barcode_for_variable_product(instance, each_var)
                                    if product_barcode_exist:
                                        ...
                                        not product_barcode_exist.ks_product_variant)):
                                            _logger.error(f'Product barcode already exist for the product of woo id {product_json_data.get("id")}.')
                                            self.env.cr.commit()
                                            return False
                            main_product_data = self.ks_map_product_template_data_for_odoo(product_json_data, instance,
                                                                                           product_exist.ks_product_template)
            ...
            if product_exported:
                try:
                    ...
                    if product_data_response:
                        ...
                        self.env['ks.woo.product.images'].ks_update_images_for_odoo(product_data_response.get("images"),
                                                                                    product=self.id)
                        self.ks_wc_variant_ids.filtered(lambda x:x.ks_wc_instance.id == instance.id).ks_manage_woo_product_variant_export(instance, queue_record=queue_record,
                                                                                    product_config=product_config)
                ...
                try:
                    ...
                    if product_data_response:
                        ...
                        self.env['ks.woo.product.images'].ks_update_images_for_odoo(product_data_response.get("images"),
                                                                                    product=self.id)
                        self.ks_wc_variant_ids.filtered(lambda x:x.ks_wc_instance.id == instance.id).ks_manage_woo_product_variant_export(instance,
                                                                                    product_config=product_config)
    ...
    def ks_prepare_product_data_to_export(self, instance, product_template, product_type=False, product_config=False,
        ...
        data = {
            ...
            "type": product_type,
            "description": self.ks_woo_description or '',
            "short_description": self.ks_woo_short_description or '',
        ...
        if self.ks_woo_product_type == "simple":
            if self.ks_product_template.product_variant_id:
                ...
                stock_qty = self.env['product.product'].ks_get_stock_quantity(self.ks_wc_instance.ks_warehouse,
                                                                              variant,
                                                                              self.ks_wc_instance.ks_stock_field_type.name)
                sale_price = self.ks_wc_instance.ks_woo_sale_pricelist.ks_get_product_price(variant)
                if server_action:
                    if product_config["price"] == True:
                        data.update({
                            "regular_price": str(
                                self.ks_wc_instance.ks_woo_regular_pricelist.ks_get_product_price(variant)),
        ...
        if instance and instance.ks_want_maps:
            if self.ks_woo_product_type == "simple":
                ...
                for map in product_maps:
                    ...
                    query = """
                        select %s from product_product where id = %s
    ...
    def create_woo_record(self, instance, odoo_product, export_to_woo=False, queue_record=False):
        """
        :param instance: ks.woo.connector.instance()
        :param odoo_product: product.template()
    ...
    def update_woo_record(self, instance, odoo_product, export_to_woo=False, queue_record=False):
        """
            :param instance: ks.woo.connector.instance()
            :param odoo_product: product.template()
            :param export_to_woo: optional, If want to directly export it or not
    ...
    def ks_map_prepare_data_for_layer(self, instance, odoo_product):
        """
        :param instance: ks.woo.connector.instance()
        :param odoo_product: product.template
        ...
        data = {
            "ks_product_template": odoo_product.id,
            "ks_wc_instance": instance.id,
            "ks_woo_product_type": 'simple' if not odoo_product.attribute_line_ids else 'variable'
        }
    ...
    def ks_map_product_template_data_for_odoo(self, json_data, instance, main_product=False):
        ...
        # image_json_data = json_data.get("images", [])
        # image = False
        # if image_json_data:
        #     main_image_url = image_json_data[0]['src']
        #     if main_image_url:
            ...
            if json_data.get("meta_data"):
                ...
                for map in product_maps:
                    ...
                    for meta_data in json_data.get("meta_data"):
                        if meta_data.get("key", '') == json_key:
                            if odoo_field == 'barcode' and len(meta_data.get("value")) == 0 and json_data.get("type") == 'simple':
                                meta_data.update({'value': False})
                                data.update({'barcode_check_field': ''})
                            elif odoo_field == 'barcode' and len(meta_data.get("value")) != 0 and json_data.get("type") == 'simple':
...
class KsProductTemplateInherit(models.Model):
    ...
    def ks_push_to_woocommerce(self):
        action = self.env.ref("ks_woocommerce.s_action_ks_export_product_template").read()[0]
        return action
...
class KsPricelistItem(models.Model):
    ...
    def compare_sale_and_regular_price_export(self):
        if self.product_tmpl_id.product_variant_count == 1:
            ks_woo_prod_temp_id = self.env.context['active_id']
            ks_wc_instance = self.env['ks.woo.product.template'].browse(self.env.context['active_id']).ks_wc_instance.id
            sale_pricelist = self.pricelist_id.search(
                [('ks_wc_instance', '=', ks_wc_instance),
                 ('id', '=', self.pricelist_id.id)]).ks_wc_sale_pricelist
            regular_pricelist = self.pricelist_id.search(
                [('ks_wc_instance', '=', ks_wc_instance),
                 ('id', '=', self.pricelist_id.id)]).ks_wc_regular_pricelist
            if sale_pricelist and self.product_tmpl_id.ks_product_template.browse(
                    ks_woo_prod_temp_id).ks_woo_rp_pricelist.fixed_price <= self.fixed_price:
                raise ValidationError(_('Sale Price must be less than Regular Price.'))
            elif regular_pricelist and self.product_tmpl_id.ks_product_template.browse(
                    ks_woo_prod_temp_id).ks_woo_sp_pricelist.fixed_price >= self.fixed_price:
                raise ValidationError(_('Regular Price must be greater than Sale Price.'))

        elif self.product_tmpl_id.product_variant_count > 1:
            ks_woo_prod_var_id = self.env.context['active_id']
            ks_wc_instance = self.env['ks.woo.product.variant'].browse(self.env.context['active_id']).ks_wc_instance.id
            sale_pricelist = self.pricelist_id.search(
                [('ks_wc_instance', '=', ks_wc_instance),
                 ('id', '=', self.pricelist_id.id)]).ks_wc_sale_pricelist
            regular_pricelist = self.pricelist_id.search(
                [('ks_wc_instance', '=', ks_wc_instance),
                 ('id', '=', self.pricelist_id.id)]).ks_wc_regular_pricelist
            if sale_pricelist and self.product_id.ks_product_variant.ks_woo_rp_pricelist.fixed_price <= self.fixed_price:
                raise ValidationError(_('Sale Price must be less than Regular Price.'))
            elif regular_pricelist and self.product_id.ks_product_variant.ks_woo_sp_pricelist.fixed_price >= self.fixed_price:
                raise ValidationError(_('Regular Price must be greater than Sale Price.'))
