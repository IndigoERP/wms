...
class KsWooProductVariant(models.Model):
    ...
    ks_woo_regular_price = fields.Char(string='Woo Regular Price', related="ks_woo_rp_pricelist.price", default="0.0")
    ks_woo_sale_price = fields.Char(string='Woo Sale Price', related="ks_woo_sp_pricelist.price", default="0.0")
    ks_product_variant = fields.Many2one('product.product', string='Odoo Product Variant', readonly=True, help="Displays Odoo Linked Product Variant Name")
    ...
    ks_woo_description = fields.Html(string="Description", help="Message displayed as product description on WooCommerce")
    ...
    @api.depends('ks_wc_instance', 'ks_wc_instance.ks_woo_regular_pricelist', 'ks_wc_instance.ks_woo_sale_pricelist', 'ks_product_variant')
    ...
    def ks_prepare_product_variant_to_export(self, instance, product_config=False):
        ...
        if server_action:
            ...
            if product_config["image"] == True and self.ks_wc_image_ids:
            ...
            data.update({
                ...
                "stock_quantity": stock_qty,
                "stock_status": "instock" if stock_qty > 0 else "outofstock",
                "regular_price": str(self.ks_wc_instance.ks_woo_regular_pricelist.ks_get_product_price(variant)),
                "sale_price": str(sale_price) if sale_price else '',
...
class KsProductVariantInherit(models.Model):
    ...
    def ks_manage_price_to_export(self, product_variant, instance):
        ...
        all_regular_price_list = instance.ks_woo_pricelist_ids.filtered(lambda l: l.ks_wc_instance == instance and (l.ks_wc_regular_pricelist))
        ...
        for price_list in all_regular_price_list:
            price_list.ks_set_product_price(product_id=product_variant.id, price=product_variant.lst_price, main_price_list=regular_price_list)
        for price_list in all_sale_price_list:
            price_list.ks_set_product_price(product_id=product_variant.id, price=0, main_price_list=sale_price_list)
    ...
    def ks_manage_price_to_import(self, instance, regular_price=0, sale_price=0):
        ...
        sale_price_list = instance.ks_woo_sale_pricelist
        all_regular_price_list = instance.ks_woo_pricelist_ids.filtered(lambda l: l.ks_wc_instance == instance and (l.ks_wc_regular_pricelist))
        all_sale_price_list = instance.ks_woo_pricelist_ids.filtered(lambda l: l.ks_wc_instance == instance and (l.ks_wc_sale_pricelist))
        for price_list in all_regular_price_list:
