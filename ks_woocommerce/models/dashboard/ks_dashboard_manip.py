...
class KsDashboard(models.Model):
            ...
            if domain_parse:
                for i in range(len(domain_parse)):
                    if isinstance(domain_parse[i], tuple):
                        ...
                        temp = tuple(temp)
                        domain_parse[i] = temp
        action = self.env.ref('ks_woocommerce.action_ks_woo_connector_instance').read()[0]
    ...
    def get_ks_variants(self):
        action = self.env.ref('ks_woocommerce.action_ks_woo_product_variants_').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_attributes(self):
        action = self.env.ref('ks_woocommerce.action_ks_woo_product_attribute').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_refunds(self):
        action = self.env.ref('ks_woocommerce.action_woocommerce_refund').read()[0]
        action['domain'] = [('move_type', '=', 'out_refund'), ('ks_woo_order_id', '!=', False)]
        return action

    def get_ks_tags(self):
        action = self.env.ref('ks_woocommerce.action_woo_product_tags').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_categories(self):
        action = self.env.ref('ks_woocommerce.action_ks_woo_product_category').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_orders(self):
        action = self.env.ref('ks_woocommerce.action_woocommerce_sale_order_quote').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action

    def get_ks_coupons(self):
        action = self.env.ref('ks_woocommerce.action_ks_woo_coupon').read()[0]
        action['domain'] = self.ks_get_domain_parse(action['domain'])
        return action
    ...
    def get_inventory(self):
        action = self.env.ref('ks_woocommerce.action_woocommerce_inventory_adjustments').read()[0]
