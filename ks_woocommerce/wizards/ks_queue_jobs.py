...
class KsQueueManager(models.TransientModel):
                ...
                try:
                    if record.ks_operation == 'woo_to_odoo':
                        _logger.info("Stock syncing from woocommerce to odoo starts for instance [%s -(%s)]",
                                     record.ks_wc_instance.ks_instance_name, record.ks_wc_instance.id)
                        product_data = json.loads(record.ks_data)
                        product_data_non_filter = self.env['ks.woo.product.template'].ks_get_product_data_for_stock_adjustment(
                        ...
                        inventory_adjustment_created = self.env['stock.inventory'].ks_create_stock_inventory_adjustment(
                            valid_product_data, record.ks_wc_instance.ks_warehouse.lot_stock_id,
                        queue_record=record)
            ...
            if record.ks_model == 'sale_order':
                ...
                try:
                    ...
                    if record.ks_operation == 'odoo_to_woo':
                        ...
                        sale_order_record.ks_export_order_to_woo(queue_record=record)
                except Exception as e:
                    record.ks_update_failed_state()
                    self.env.cr.commit()
                    _logger.info(str(e))
    ...
    def ks_create_order_record_in_queue(self, instance=False, data=False, records=False):
        vals = []
        if data:
            for record in data:
                woo_id = record.get("id")
